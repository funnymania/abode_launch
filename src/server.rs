...
impl Server {
    ...
    pub fn handle_https<T: Read + Write>(
        ...
        //IMP: Implement the ability to limit use of this token by IP or Host.
        //API token only used for testing.
        let wuh_token = Uuid::parse_str("00ec183d-7446-4511-b624-4349e385d730").unwrap();
        Server::add_token(&mut client, &wuh_token);
        ...
        match stream.read(&mut req) {
            ...
            Ok(bytes_read) => {
                ...
                match std::str::from_utf8(&req) {
                    ...
                    Err(e) => {
                        ...
                        return;
                                        ...
                                        let string_req = Server::print_bytes(&req);
                                        let cookie_map = Server::retrieve_cookies(&string_req);
                ...
                match str_req.1.as_str() {
                    ...
                    "/edit-profile" => {
                        let content = match Server::get_page("/views/edit-profile.html") {
                                Ok(html) => html,
                                Err(e) => format!("<html><body>Webpage was not formatted correctly, please @funnymania_ in case they are sleeping (Zzzz):<br><a href=\"https://twitter.com/funnymania_\">https://twitter.com/funnymania_</a><br><br>Error: {}</body></html>", e)
                            };
                        response = format!(
                    ...
                    "/user-profile" => {
                        let content = match Server::get_page("/views/user-profile.html") {
                                Ok(html) => html,
                                Err(e) => format!("<html><body>Webpage was not formatted correctly, please @funnymania_ in case they are sleeping (Zzzz):<br><a href=\"https://twitter.com/funnymania_\">https://twitter.com/funnymania_</a><br><br>Error: {}</body></html>", e)
                            };
                        response = format!(
                                            ...
                                            "identity" => {
                                                match str_req.0.as_str() {
                                                    "GET" => {
                                                        ...
                                                        let token = "3950ce00-a853-4cbe-ba19-b44488ad4314";
                                                    ...
                                                    "PUT" => match Server::extract_body(&req) {
                                                        Ok(body) => {
                                                            let mut status_code = String::new();
                                                            let mut token = match cookie_map.get("sess_token") {
                                                                Some(token_v) => String::from(token_v),
                                                                None => String::from(""),
                                                            };
    ...
    pub fn retrieve_cookies(request: &str) -> HashMap<String, String> {
       let mut cookie_map = HashMap::new();

       //Break request into lines
        let lines: Vec<&str> = request.lines().collect();

        let mut cookie_word: Vec<&str> = Vec::new();
        for line in lines {
            let mut words: Vec<&str> = line.split(" ").collect();
            if words.len() > 0 && words[0] == "Cookie:" {
                cookie_word = words.split_off(1); 
                break;
            }
        }

        for cookie in cookie_word {
            let stripped_cookie = cookie.replace(";", "");
            let kv_cookie:Vec<&str> = cookie.split("=").collect();
           cookie_map.insert(kv_cookie[0].to_string(), kv_cookie[1].to_string());
        }
    ...
    pub fn increment_gets(client: &mut Arc<Mutex<postgres::Client>>, api_token: &Uuid) -> Result<json::JsonValue, json::JsonValue> {
        ...
        match client.execute("UPDATE api_token SET get_user = get_user + 1 WHERE id = $1 RETURNING *", &[api_token]) {
            Ok(res) => {
                if res == 0 {
                    Err(object! { error: String::from("Token does not exist")})
                } else {
                    Ok(object! { success: String::from("Record updated") })
                }
            }
            Err(e) => {
                Err(object! { error: String::from("Formatting error.")})
            }
        }
    }
    ...
    pub fn increment_registered_users(client: &mut Arc<Mutex<postgres::Client>>, api_token: &Uuid)  -> Result<json::JsonValue, json::JsonValue> {
        ...
        match client.execute("UPDATE api_token SET reg_user = reg_user + 1 WHERE id = $1", &[api_token]) {
            Ok(rows) => {
                if rows == 0 {
    ...
    pub fn increment_updates(client: &mut Arc<Mutex<postgres::Client>>, api_token: &Uuid)  -> Result<json::JsonValue, json::JsonValue> {
        ...
        match client.execute("UPDATE api_token SET update_user = update_user + 1 WHERE id = $1", &[api_token]) {
            Ok(rows) => {
                if rows == 0 {
                    Err(object! { error: String::from("API token not found.")})
                } else {
    ...
    //IMP: user_identity table (id_id, user_id, name, configs, attributes) is the table. get_user
    ...
    pub fn print_bytes(req: &[u8]) -> String {
        let req_str = String::from_utf8_lossy(req);
        let trimmed = req_str.trim_matches(char::from(0));
        println!("{}", trimmed);

        String::from(trimmed)
    }

    //IMP: Multi-device login
                    ...
                    if rows.len() == 0 {
                        ...
                        match other_logins {
                            Ok(token_rows) => {
                                ...
                                //IMP: Default ID 0 = (unknown), 1 = iphone, 2 = android, 3 = mac,
                                //4 = linux, 5 = windows
                                ...
                                match insert_res {
                                    Ok(_) => {
                                        //IMP: Handle multiple identities (right now just
                                        //returning first).
                                        match client
    ...
    pub fn add_token(client: &mut Arc<Mutex<postgres::Client>>, token: &Uuid) {
        let mut client = client.lock().unwrap();
        let zero = 0i64;
        match client.execute("INSERT INTO api_token (id, get_user, reg_user, update_user) VALUES ($1, $2, $3, $4)", &[token, &zero ,&zero ,&zero]) {
            Err(e) => println!("Could not insert: {}", e),
            _ => {}
        }
    }
